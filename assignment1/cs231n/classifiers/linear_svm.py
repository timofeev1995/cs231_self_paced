...
def svm_loss_vectorized(W, X, y, reg):
    ...
    scores = np.dot(X, W)
    true_class_scores = scores[range(num_train), y].reshape(-1, 1)
    margins = scores - true_class_scores + 1
    ...
    margin_condition_mask = (margins > 0).astype('int')
    margin_condition_mask[range(num_train), y] -= margin_condition_mask.sum(axis=1)
    
    dW = np.dot(X.T, margin_condition_mask)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
